...
#define UNRECOVERABLE_IF(expression)                                                            \
                                                                                                \
    if (expression) {                                                                           \
         std::cout << "Abort was called at " << __LINE__ << " line in file: " << __FILE__ << std::endl; \
         std::abort();                                                                          \
    }
...
#define ZE_GEN_PER_API_CALLBACK_STATE(perApiCallbackData, tracerType, callbackCategory, callbackFunctionType)                               \
    ...
    currentTracerArray = (tracing_layer::tracer_array_t *)tracing_layer::pGlobalAPITracerContextImp->getActiveTracersList();                                      \
